...
# 测试函数
def test_hypothesis_extractor():
    """测试假设提取器"""
    print("=== 测试 HypothesisExtractor ===")
...
"""
    ...
    hypotheses = extractor.extract_hypotheses(test_code, static_info)
    
    print(f"提取到 {len(hypotheses)} 个假设:")
    for i, hypothesis in enumerate(hypotheses, 1):
        print(f"\n假设 {i}:")
        print(f"  CWE: {hypothesis.get('cwe')}")
        print(f"  类型: {hypothesis.get('type')}")
        print(f"  置信度: {hypothesis.get('confidence')}")
        print(f"  描述: {hypothesis.get('description', '')[:100]}")
        if hypothesis.get('evidence'):
            print(f"  证据: {hypothesis['evidence'][:2]}")
    
    return hypotheses

if __name__ == "__main__":
    test_hypothesis_extractor()